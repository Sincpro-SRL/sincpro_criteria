...
export function validate<T = AnyRecord>(written: CriteriaLike<T>, meta: Meta): Validation<T> {
  ...
  const levels = criteria.grouping?.group_by;
  if (levels !== undefined) {
    ...
    checked.grouping = { ...criteria.grouping, group_by: kept };
  }

  const totals = new Set([...Object.keys(criteria.grouping?.measures ?? {}), "count"]);
  for (const name of namesIn(criteria.grouping?.where_measures)) {
    if (!totals.has(name)) {
      refused.push({
        field: name,
