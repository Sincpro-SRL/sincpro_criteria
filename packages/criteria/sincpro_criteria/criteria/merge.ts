...
export function merge<T = AnyRecord>(
  ...
  const where = combined<T>(mine.where, theirs.where);
  ...
  const grouping = theirs.grouping?.group_by?.length ? theirs.grouping : mine.grouping;
...
/**
 * The reading as it should be SAVED: everything it asks for, without the cursor.
 *
 * A cursor names a row under one ordering over one filter and does not outlive either. Saved
 * inside a reading it comes back stale — the reading recovers by reading the first page
 * again, but a saved reading should not have to be recovered from in the first place.
 *
 * @example
 * localStorage.setItem("my-reading", pack(forSaving(reading.criteria)));
 */
export function forSaving<T = AnyRecord>(criteria: Criteria<T>): Criteria<T> {
  const { limit } = criteria.pagination ?? {};
  const pagination = limit === undefined ? undefined : { limit };
  const saved = { ...criteria };
  if (pagination === undefined) delete saved.pagination;
  else saved.pagination = pagination;
  return saved;
}
