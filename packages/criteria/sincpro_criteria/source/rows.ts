 ...
 * to a server, so the day it stops bringing everything down, only the source changes. It
 * answers `read`, `group` and `pivot`, because everything it needs is right here.
 *
 * @example
 * const datasets = resource(fromRows(rows, { identity: "dataset_id" }));
 * const page = await datasets.page(criteria().where(["encoding", "=", "utf-8"]).limit(20));
 */
