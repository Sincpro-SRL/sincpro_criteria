...
import { criteria, measure, readFilter, validate, where } from "@sincpro/criteria";
...
describe("what the model cannot answer is dropped, never refused", () => {
  it("drops a field the model does not have", () => {
    const { criteria: checked, dropped } = validate(
      criteria<Dataset>().where(
        ["encoding", "=", "utf-8"],
        ["legacy_flag" as never, "=", true],
      ),
      datasetMeta,
    );
    assert.deepEqual(dropped, [{ field: "legacy_flag", reason: "unknown_field" }]);
    assert.deepEqual(checked.where, readFilter(["encoding", "=", "utf-8"]));
  });

  it("drops an operator the type does not take", () => {
    const { dropped } = validate(
      criteria<Dataset>().where(["row_count", "like", "1000"]),
      datasetMeta,
    );
  ...
  it("drops a value that will not read as the field's type", () => {
    const { dropped } = validate(
      criteria<Dataset>().where(["row_count", ">", "banana"]),
      datasetMeta,
    );
  ...
  it("drops a grouping level that points at another aggregate", () => {
    const { dropped } = validate(criteria<Dataset>().groupBy("runs" as never), datasetMeta);
...
describe("the tree collapses around what it lost", () => {
  it("removes a connective that lost every part, rather than leaving it empty", () => {
    const { criteria: checked } = validate(
      criteria<Dataset>().where(
        where.any(["gone" as never, "=", 1], ["also_gone" as never, "=", 2]),
      ),
      datasetMeta,
    );
  ...
  it("unwraps a connective left with one part", () => {
    const { criteria: checked } = validate(
      criteria<Dataset>().where(
        where.all(["encoding", "=", "utf-8"], ["gone" as never, "=", 1]),
      ),
      datasetMeta,
    );
    assert.deepEqual(checked.where, readFilter(["encoding", "=", "utf-8"]));
...
describe("values are read into the type the column compares against", () => {
  it("reads the text a URL carries", () => {
    const { criteria: checked } = validate(
      criteria<Dataset>().where(["row_count", ">", "1000"]),
      datasetMeta,
    );
    assert.deepEqual(checked.where, readFilter(["row_count", ">", 1000]));
  });

  it("reads every member of a multi-valued operator", () => {
    const { criteria: checked, dropped } = validate(
      criteria<Dataset>().where(["row_count", "between", ["10", "20"]]),
      datasetMeta,
    );
    assert.deepEqual(dropped, []);
    assert.deepEqual(checked.where, readFilter(["row_count", "between", [10, 20]]));
  });
  ...
  it("takes the value of is null as the question, not as the field's type", () => {
    const { criteria: checked, dropped } = validate(
      criteria<Dataset>().where(["producer_id", "is null", true]),
      datasetMeta,
    );
    assert.deepEqual(dropped, []);
    assert.deepEqual(checked.where, readFilter(["producer_id", "is null", true]));
  });

  it("asks about one member of a list column", () => {
    const { dropped } = validate(
      criteria<Dataset>().where(["tags", "contains", "raw"]),
      datasetMeta,
    );
    assert.deepEqual(dropped, []);
  });
...
describe("what the server would refuse outright", () => {
  ...
  it("names an ordering field the model does not have", () => {
    const { refused } = validate(criteria<Dataset>().order("gone" as never), datasetMeta);
  ...
  it("names a having that reads something the grouping does not fold", () => {
    const { refused } = validate(
      criteria<Dataset>()
        ...
        .whereMeasures(["pages" as never, ">", 10]),
      datasetMeta,
    );
  ...
  it("says nothing about a having that reads count or a total it folds", () => {
    const { refused } = validate(
      criteria<Dataset>()
        .groupBy("encoding")
        .measures({ rows: measure.sum("row_count") })
        .whereMeasures(where.all(["count", ">", 10], ["rows", ">", 100])),
      datasetMeta,
    );
...
describe("a node inside an expanded relation", () => {
  ...
  it("reports what the other side cannot answer, by its path", () => {
    const { dropped } = validate(
      criteria().specification({ runs: { where: readFilter(["gone", "=", 1]) } }),
      withRelation,
    );
  ...
  it("keeps the node it could check, cut to what survived", () => {
    const { criteria: checked } = validate(
      criteria().specification({
        runs: { where: where.all(["state", "=", "done"], ["gone", "=", 1]) },
      }),
      withRelation,
    );
    assert.deepEqual(checked.specification?.runs?.where, readFilter(["state", "=", "done"]));
  });
});
...
describe("the criteria that comes back is the one the engine will answer", () => {
  it("does not keep a grouping level it just dropped", () => {
    const { criteria: checked, dropped } = validate(
      criteria<Dataset>().groupBy("runs" as never),
      datasetMeta,
    );
    assert.deepEqual(dropped, [{ field: "runs", reason: "unsupported_operator" }]);
    assert.deepEqual(checked.grouping?.group_by, []);
  });
  ...
  it("keeps the levels it can answer", () => {
    const { criteria: checked } = validate(
      criteria<Dataset>()
        .groupBy("encoding")
        .groupBy("runs" as never),
      datasetMeta,
    );
    assert.deepEqual(checked.grouping?.group_by, [{ field: "encoding" }]);
  });
});
...
describe("a blank box in a filter form", () => {
  it("is not a zero", () => {
    const { criteria: checked, dropped } = validate(
      criteria<Dataset>().where(["row_count", ">", ""]),
      datasetMeta,
    );
    assert.deepEqual(dropped, [{ field: "row_count", reason: "bad_value" }]);
    assert.equal(checked.where, undefined);
