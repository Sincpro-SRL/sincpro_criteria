...
describe("a definition that has not arrived yet", () => {
  it("answers empty and undefined, never throws, for null or undefined meta", () => {
    for (const meta of [null, undefined] as const) {
      assert.deepEqual(operatorsFor(meta, "encoding"), []);
      assert.deepEqual(fieldsThatFilter(meta), []);
      assert.deepEqual(fieldsThatSort(meta), []);
      assert.deepEqual(fieldsThatGroup(meta), []);
      assert.equal(labelOf(meta, "row_count"), "row_count"); // the field's own name, as-is
      assert.equal(nameOf(meta), ""); // no aggregate name to fall back to either
      assert.equal(searchOver(meta, "labs"), undefined);
    }
  });
});
