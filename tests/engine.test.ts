...
describe("a page out of rows in hand", () => {
  it("filters, sorts, pages and counts", () => {
    const page = answer(
      rows,
      criteria<Dataset>().where(eq("encoding", "utf-8")).order("row_count").limit(2),
      datasetMeta,
    );
  ...
  it("resumes exactly where the previous page ended", () => {
    ...
    const two = answer(rows, first.resumingFrom(one.cursor), datasetMeta);
    const three = answer(rows, first.resumingFrom(two.cursor), datasetMeta);
    ...
    for (const limit of [1, 2, 3, 5, 9]) {
      ...
      for (let guard = 0; guard < 20; guard += 1) {
        const page: ReturnType<typeof answer<Dataset>> = answer(
          rows,
          cursor === null ? base : base.resumingFrom(cursor),
          datasetMeta,
        );
  ...
  it("counts rows instead, when a grid jumps to a page", () => {
    const page = answer(
      rows,
      criteria<Dataset>().order("dataset_id").limit(2).skipping(2),
      datasetMeta,
    );
  ...
  it("drops what the model cannot answer and still runs", () => {
    const page = answer(rows, criteria<Dataset>().where(eq("gone" as never, 1)), datasetMeta);
...
describe("grouping", () => {
  it("counts and folds every bucket, and carries the criteria that opens it", () => {
    const page = answerGroups(
      ...
      criteria<Dataset>()
        .by("encoding")
        .totals({ total: ["sum", "row_count"] }),
      datasetMeta,
    );
    assert.deepEqual(
      page.rows.map((one) => [one.value, one.count, one.totals?.total]),
      [
        ["latin-1", 2, 750],
        ["utf-8", 3, 10500],
      ],
  ...
  it("splits a date by the grain, and opens the bucket with a range", () => {
    const page = answerGroups(
      rows,
      criteria<Dataset>().by({ field: "registered_at", grain: "month" }),
      datasetMeta,
    );
    assert.deepEqual(
  ...
  it("answers a page of ids per bucket when the criteria asked for a page", () => {
    const page = answerGroups(
      rows,
      criteria<Dataset>().by("encoding").order("row_count").limit(1),
      datasetMeta,
    );
    const utf8 = page.rows.find((one) => one.value === "utf-8")!;
  ...
  it("answers no ids when no page was asked for", () => {
    const page = answerGroups(rows, criteria<Dataset>().by("encoding"), datasetMeta);
...
describe("a pivot", () => {
  it("crosses two axes and folds the cells", () => {
    ...
    assert.equal(cellOf(pivot, ["utf-8"], ["2026-01"])?.totals?.total, 1500);
    assert.equal(
  ...
  it("folds its margins from the rows, never from the cells", () => {
    const pivot = pivotOf(rows, {
      rows: ["encoding"],
      columns: [],
      totals: { average: ["avg", "row_count"] },
    });
    assert.equal(pivot.total?.count, 5);
    assert.equal(
      pivot.total?.totals?.average,
      2250,
