...
describe("a criteria in a URL", () => {
  ...
  it("is safe in a URL", () => {
    const packed = pack(criteria().where(eq("name", "a/b+c?d=e")));
...
describe("reading a criteria that came from outside", () => {
  ...
  it("refuses a fold the engine cannot compute", () => {
    assert.throws(
      () => readCriteria({ grouping: { totals: { rows: ["median", "row_count"] } } }),
      InvalidCriteria,
    );
  });
  ...
  it("refuses a grain it cannot cut", () => {
    assert.throws(
      () =>
        readCriteria({ grouping: { by: [{ field: "registered_at", grain: "fortnight" }] } }),
      InvalidCriteria,
    );
  });

  it("refuses a level written as a bare string, because the wire has one spelling", () => {
    assert.throws(() => readCriteria({ grouping: { by: ["encoding"] } }), InvalidCriteria);
  });
