...
import { criteria, eq, gt, like } from "@sincpro/criteria";

import type { Dataset } from "./fixtures";

describe("the builder", () => {
  it("is the plain object and nothing else", () => {
    const q = criteria<Dataset>().where(eq("encoding", "utf-8")).limit(100);
  ...
  it("never touches the one it came from", () => {
    ...
    const refined = base.where(like("name", "labs"));
  ...
  it("accumulates the filter with AND, flat", () => {
    const q = criteria<Dataset>().where(eq("encoding", "utf-8")).where(gt("row_count", 1000));
    assert.deepEqual(q.value.where, {
      all: [eq("encoding", "utf-8"), gt("row_count", 1000)],
    });
  });
...
describe("the ordering", () => {
  it("reads the line a URL carries", () => {
...
describe("the page", () => {
  it("keeps the size when it resumes", () => {
    const q = criteria<Dataset>().limit(80).resumingFrom("eyJrIjo");
    assert.deepEqual(q.value.pagination, { limit: 80, strategy: { token: "eyJrIjo" } });
  });

  it("counts rows when a grid jumps to a page", () => {
    assert.deepEqual(criteria<Dataset>().limit(25).skipping(150).value.pagination, {
      limit: 25,
      strategy: { rows: 150 },
    });
  });
...
describe("the grouping", () => {
  it("accumulates its levels, in order", () => {
    const q = criteria<Dataset>()
      .by("encoding")
      .by({ field: "registered_at", grain: "month" });
    assert.deepEqual(q.value.grouping?.by, [
      { field: "encoding" },
  ...
  it("adds the folds without losing the levels", () => {
    const q = criteria<Dataset>()
      ...
      .totals({ biggest: ["max", "row_count"] });
    assert.deepEqual(q.value.grouping, {
      by: [{ field: "encoding" }],
      totals: { rows: ["sum", "row_count"], biggest: ["max", "row_count"] },
    });
  });
  ...
  it("orders and pages the buckets without touching the rest", () => {
    const q = criteria<Dataset>()
      .by("encoding")
      .grouping({ order: [{ field: "count", descending: true }], pagination: { limit: 10 } });
    assert.deepEqual(q.value.grouping?.by, [{ field: "encoding" }]);
