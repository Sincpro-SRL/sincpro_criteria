...
import { criteria, eq, gt, merge, narrowedBy, resumingFrom } from "@sincpro/criteria";
...
describe("the law of merging two criteria", () => {
  it("ANDs the filters, because both restrictions hold", () => {
    ...
    assert.deepEqual(merged.where, { all: [eq("a", 1), gt("b", 2)] });
  });

  it("keeps the filter flat however many times it is merged", () => {
    const once = merge({ where: eq("a", 1) }, { where: eq("b", 2) });
    const twice = merge(once, { where: eq("c", 3) });
    assert.deepEqual(twice.where, { all: [eq("a", 1), eq("b", 2), eq("c", 3)] });
  });
  ...
  it("keeps this one's page when the other said nothing about it", () => {
    const merged = merge({ pagination: { limit: 20 } }, { where: eq("a", 1) });
  ...
  it("NEVER carries a cursor over, because the set it named may no longer exist", () => {
    ...
    const merged = merge(mine, { where: eq("a", 1) });
    assert.deepEqual(merged.pagination, { limit: 20 });
  });
  ...
  it("takes the other's grouping only when it asked to group", () => {
    const mine = { grouping: { by: [{ field: "encoding" }] } };
    assert.deepEqual(merge(mine, {}).grouping, mine.grouping);
    assert.deepEqual(merge(mine, { grouping: { by: [{ field: "name" }] } }).grouping, {
      by: [{ field: "name" }],
    });
  });
...
describe("merging through the builder", () => {
  it("is the same law", () => {
    const saved = criteria().where(eq("a", 1)).limit(20);
    const typed = criteria().where(gt("b", 2));
    assert.deepEqual(saved.merge(typed).value, {
      where: { all: [eq("a", 1), gt("b", 2)] },
      pagination: { limit: 20 },
    });
  });
});
