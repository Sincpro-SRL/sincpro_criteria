...
import { all, any, criteria, eq, explain, gt, isNull } from "@sincpro/criteria";
...
describe("a criteria in words", () => {
  ...
  it("uses the model's own words when it has them", () => {
    const q = criteria<Dataset>().where(gt("row_count", 1000));
    assert.equal(
  ...
  it("parenthesises a group so an or does not read as an and", () => {
    const q = criteria<Dataset>().where(
      all(eq("encoding", "utf-8"), any(gt("row_count", 10), isNull("producer_id"))),
    );
    assert.equal(
      explain(q),
  ...
  it("says what a grouping counts", () => {
    ...
    assert.equal(explain(q), "grouped by encoding then registered_at by month, folding rows");
  });
