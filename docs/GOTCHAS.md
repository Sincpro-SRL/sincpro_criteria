...
- **Symptom:** `all(eq("a", 1), eq("b", 2))` is refused with "Argument of type `"b"` is not
  assignable to parameter of type `"a"`"; or the other way round, `eq("encodig", …)` compiles
  against a typed builder.
