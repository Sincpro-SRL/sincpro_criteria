/**
 * A model definition read off the rows themselves.
 *
 * The engine derives its definition from the mapped class and sends it with every answer;
 * when the rows came from somewhere that says nothing about them — a CSV, a fixture, a
 * response from a service that does not speak this vocabulary — this reads one off the rows,
 * so a filter form, a column menu and a validation still have something to stand on.
 *
 * It is a guess, and it says so by looking at every row rather than the first: a column is
 * nullable when some row leaves it empty, and a column is text when some row writes text in
 * it. What it cannot see, it does not invent — there are no relations here.
 *
 * @module
 */
