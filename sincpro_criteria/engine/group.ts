/**
 * Splitting rows into buckets, one level at a time.
 *
 * Every bucket carries **the criteria that opens it** — this reading plus "and this bucket's
 * value" — so seeing its rows is a plain read with that criteria and nothing else. No client
 * rebuilds a filter it did not write, and a screen that drills down one click at a time asks
 * for one level and reads the bucket's criteria on the click.
 *
 * @module
 */

import { all, eq, gte, lt } from "@sincpro/criteria/criteria/expression";
import type {
  AnyRecord,
  Criteria,
  Expression,
 ...
 *
 * @example
 * bucketsOf(rows, criteria().by("encoding").totals({ rows: ["sum", "row_count"] }).value, meta);
 */
export function bucketsOf<T = AnyRecord>(
  ...
  if (grouping?.by === undefined || grouping.by.length === 0) return [];
  return tree(rows, grouping, criteria, grouping.by, meta);
}
...
function tree<T>(
  ...
  for (const { value, range, rows: mine } of held.values()) {
    const numbers = folded(mine, grouping.totals);
    ...
    if (grouping.having !== undefined) {
      ...
      if (!matches(answered, grouping.having as Expression)) continue;
    }
...
function conditionFor<T>(
  ...
  if (range === undefined) return eq<T>(level.field, value as never);
  return all<T>(gte<T>(level.field, range.from), lt<T>(level.field, range.to));
}
...
function compareByName<T>(mine: Bucket<T>, theirs: Bucket<T>, name: string): number {
  ...
  if (mine.totals !== undefined && name in mine.totals) {
    return compareValues(mine.totals[name], theirs.totals?.[name]);
  }
