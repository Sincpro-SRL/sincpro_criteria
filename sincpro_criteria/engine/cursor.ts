/**
 * Where a page ended, and how the next one resumes from it.
 *
 * The token is the engine's own: base64url of `{"k": [...keys], "o": "the ordering"}`, with
 * the types JSON cannot carry tagged. Writing the same string matters — a source that pages
 * in memory and a source that pages in a database are interchangeable only if the cursor that
 * comes out of one can go into the other.
 *
