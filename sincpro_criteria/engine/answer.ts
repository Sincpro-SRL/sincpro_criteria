/**
 * The whole read, over rows already in hand: check, filter, sort, page, count.
 *
 * This is the engine's `search` with no database under it. It is what makes a list already
 * loaded a first-class source — the same criteria that asks a server can ask an array, and
