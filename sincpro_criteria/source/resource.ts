/**
 * A source, with everything this package can do to it.
 *
 ...
 *   read: async (criteria) => pageFrom(await getJson("/datasets", { criteria: pack(criteria) })),
 * });
 *
 * const reading = datasets.read(criteria<Dataset>().order("-registered_at").limit(100));
 * await reading.load();
 *
 * for await (const dataset of datasets.all(criteria<Dataset>().where(eq("encoding", "utf-8")))) {
